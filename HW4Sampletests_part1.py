...
class HW4Sampletests_part1(unittest.TestCase):
    ...
    def test_userCountToMark(self): #will print out 5 because there is no mark so it won't be popping any values in the counttomark() function
        opPush(4)
        opPush(5)
        counttomark()
    ...
    def test_userCleartomask(self): #wont pop anything from the stack because there is no mark in the stack
        opPush(4)
        opPush(5)
        opPush(1)
        cleartomark()
        self.assertTrue(opPop() == 1 and opPop() == 5 and opPop() == 4)
    ...
    def test_userGet(self): #user test case
        #[True False 4 5 3] 2 get
        opPush(5)
        opPush([True,False,4,5,3])
        opPush(2)
        get()
        self.assertEqual(opPop(), 4)
    ...
    def test_getinterval(self):
        ...
        self.assertTrue(len(opstack)==0)        
    def test_userGetInterval(self): # user test case
        #[True False "helloworld" 5 4] 3 get
        opPush([True,False,"helloworld",5,4])
        opPush(1)
        opPush(3)
        getinterval()
        self.assertEqual(opPop(), [False, "helloworld", 5])
    ...
    def test_userPut(self): #user test case. Should return 4 because there is no array so it won't update any array and it will push the bools and int back to the stack
        opPush(False)
        opPush(True)
        opPush(4)
        put()
    ...
    def test_userPutInterval(self): #user test case. Should return none because the list is less than 2 elements
        opPush([False])
        dup()
        opPush(21)
        putinterval()
    ...
    def test_userBeginEnd(self):
        ...
        self.assertEqual(lookup('z'), None)
